...
(function(c,f,g){c.tapHandling=false;var d=function(h){return h.each(function(){var s=f(this),m,p,q,r,i=10;function k(t){f(t.target).trigger("tap",[t,f(t.target).attr("href")]);t.stopImmediatePropagation()}function o(v){var t=v.originalEvent||v,u=t.touches||t.targetTouches;if(u){return[u[0].pageX,u[0].pageY]}else{return null}}function j(u){if(u.touches&&u.touches.length>1||u.targetTouches&&u.targetTouches.length>1){return false}var t=o(u);q=t[0];p=t[1]}function l(u){if(!r){var t=o(u);if(t&&(Math.abs(p-t[1])>i||Math.abs(q-t[0])>i)){r=true}}}function n(t){clearTimeout(m);m=setTimeout(function(){c.tapHandling=false;r=false},1000);if((t.which&&t.which>1)||t.shiftKey||t.altKey||t.metaKey||t.ctrlKey){return}t.preventDefault();if(r||c.tapHandling&&c.tapHandling!==t.type){r=false;return}c.tapHandling=t.type;k(t)}s.bind("touchstart.tappy MSPointerDown.tappy",j).bind("touchmove.tappy MSPointerMove.tappy",l).bind("touchend.tappy MSPointerUp.tappy",n).bind("click.tappy",n)})};var a=function(h){return h.unbind(".tappy")};if(f.event&&f.event.special){f.event.special.tap={add:function(h){d(f(this))},remove:function(h){a(f(this))}}}else{var b=f.fn.bind,e=f.fn.unbind;f.fn.bind=function(h){if(/(^| )tap( |$)/.test(h)){d(this)}return b.apply(this,arguments)};f.fn.unbind=function(h){if(/(^| )tap( |$)/.test(h)){a(this)}return e.apply(this,arguments)}}}(this,jQuery));(function(){$("html").addClass("transitions");function d(){$("a.tappilyTap").bind("tap",function(h){window.location=h.target.href})}function g(){var h=$(".js-tapTarget");if(h.length){h.bind("tap",function(){var i=$(this);window.location=i.find("a.js-tapSrc").attr("href")})}}function a(){$(".jumper").bind("tap",function(i){if(location.pathname.replace(/^\//,"")==this.pathname.replace(/^\//,"")&&location.hostname==this.hostname){var h=$(this.hash);h=h.length&&h||jQuery("[name="+this.hash.slice(1)+"]");if(h.length){var j=h.offset().top-24;$("html,body").animate({scrollTop:j},400)}}})}function f(){if($(".m-toggled").length){var i=$(".m-toggled");i.each(function(){var k=$(this);if(k.is("ul")){var j=k.clone(true);j.find("li").slice(1).wrapAll('<li class="m-toggled__elem s-is-hidden"><ul></ul></li>');j.find("li:eq(0) > *").addClass("a-toggled__toggler");k.replaceWith(j)}});var h=$(".m-toggled__elem").prev();h.bind("tap",function(){$this=$(this);var k=$this.parent(".m-toggled");var j=$this.next(".m-toggled__elem");k.toggleClass("s-is-active");j.slideToggle("fast").removeClass("s-is-hidden")})}}function c(){if($(".m-expand-section").length){var i=$(".m-expand-section");var h=i.innerHeight();var j=i.attr("data-divide-by");var k=i.attr("data-expand-text");var l=$('<button class="a-expand-section__button">'+k+"</button>");i.css("max-height",h/j);l.insertAfter(i);l.bind("tap",function(){$this=$(this);$this.prev(i).css("max-height",h).addClass("s-is-expanded");$this.addClass("s-is-hidden")})}}function e(){if($(".m-form").length){function h(){$("input:checked").parent(".a-label").addClass("s-is-checked");$("input:disabled").parent(".a-label").addClass("s-is-disabled");$('form[role="form"]').on("click",".a-label.checkbox, .a-label.radio",function(){$this=$(this);if($this.children().is(":disabled")){return false}var i=$this.children("input:checkbox");if(i.length){if(i.is(":checked")){$this.addClass("s-is-checked")}else{$this.removeClass("s-is-checked")}}else{var j=$this.siblings();j.removeClass("s-is-checked");$this.addClass("s-is-checked")}})}h()}}function b(){var i=0;window.watchResize=function(l){var k;function j(){clearTimeout(k);k=null;l()}$(window).on("resize",function(){if(k){clearTimeout(k);k=null}k=setTimeout(j,50)});l()};window.watchResize(function(){i=h();if(i=="bq"){alert(i)}});function h(){var k=window.getComputedStyle,j=document.getElementById("getActiveMQ-watcher");if("currentStyle" in j){h=function(){return j.currentStyle.fontFamily}}else{if(k){h=function(){return k(j,null).getPropertyValue("font-family").replace(/['"]/g,"")}}else{h=function(){return"unknown"}}}return h()}}d();a();f();c();e();b()})();